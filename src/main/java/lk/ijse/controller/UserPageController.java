...
public class UserPageController implements Initializable {
    ...
    private boolean validateFields(boolean isUpdate) {
        ...
        if (username.isEmpty() || email.isEmpty() || roleName == null || status == null) {
            new Alert(Alert.AlertType.WARNING, "Username, Email, Role, and Status are required.").show();
            return false;
        }

        if (!isUpdate && password.isEmpty()) {
            new Alert(Alert.AlertType.WARNING, "Password is required for new users.").show();
            return false;
        }
    ...
    void btnUpdateOnAction(ActionEvent event) {
        ...
        String username = txtUsername.getText().trim();
        String email = txtEmail.getText().trim();
        String password = txtPassword.getText().isEmpty() ? selectedUser.getPassword() : txtPassword.getText();
        String roleName = cmbRole.getValue();
        String status = cmbStatus.getValue();
